...
이 공개 surface만으로도 서버 중심 AI 채팅을 실제 앱에 붙일 수 있습니다.
...
실제로 이 조합만으로 아래를 구현할 수 있음을 확인했습니다.

- thread별 대화 실행
- `thread.start()` 기반 비동기 run 시작 + 즉시 ack 반환
- `thread.history()` 기반 메시지 복원
- SSE로 `run.*`, `message.*`, `step.changed` 이벤트를 앱 전용 payload로 직렬화
- requirement override 선택 UI

다만 코어가 직접 제공하지 않는 것도 분명합니다.
...
즉, 앱에서는 보통 아래 둘 중 하나가 필요합니다.

- thread 인덱스를 별도로 유지
- `ConversationStore`를 감싼 custom store를 두고 thread 메타데이터를 같이 관리

추가 메모:

- `ConversationStore` 공개 계약에는 `listThreads()`가 없습니다.
- `thread.start()`는 background run을 바로 시작하기 좋고, 실시간 UI에는 이 경로가 더 잘 맞습니다.
- 실시간 갱신은 충분히 가능하지만, 토큰 단위 증분 스트리밍은 transport 구현에 달려 있으므로 SDK 차원에서 보장하지 않습니다.
- `@zootopiafresh/agent-core@0.1.4`의 runner는 포트만 열려 있다고 기존 Gateway를 재사용하지 않고, `OPENCLAW_GATEWAY_TOKEN`으로 probe가 성공할 때만 재사용합니다.

더 자세한 앱 통합 메모는 [../../docs/agent-core-app-integration.md](../../docs/agent-core-app-integration.md)를 참고하세요.
